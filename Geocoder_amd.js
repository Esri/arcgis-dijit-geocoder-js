require([
...
], function(declare, parser, ready, _WidgetBase, _OnDijitClickMixin, _TemplatedMixin, on, array, query, domGeom, json, i18n, domConstruct, locator, template){
    declare("esri.dijit.Geocoder", [_WidgetBase, _OnDijitClickMixin, _TemplatedMixin], {
            ...
            startup: function () {
                ...
                if (this.domNode && this.map) {
                    ...
                    if (this.geocoderMenu && (this._geocoder.length < 2)) {
                        domConstruct.destroy(this.geocoderMenuArrowNode);
                    }
            ...
            _checkStatus: function () {
                ...
                if (this.value) {
                    // set class and title
                    query(this.clearNode).addClass(this._clearButtonActiveClass).attr('title', this.i18n.Geocoder.main.clearButtonTitle);
                } else {
                    // clear address
                    this.clear();
                }
            ...
            _setDelegations: function () {
                ...
                this._delegations = [];
                // close on click
                var closeOnClick = on(document, "click", instance._hideResultsMenu());
                this._delegations.push(closeOnClick);
                // input key up
                var inputKeyUp = on(this.inputNode, "keyup", instance._inputKeyUp());
                this._delegations.push(inputKeyUp);
                // input key down
                var inputKeyDown = on(this.inputNode, "keydown", instance._inputKeyDown());
